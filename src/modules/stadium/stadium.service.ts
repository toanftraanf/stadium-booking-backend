...
export class StadiumService {
  ...
  async findByName(name: string): Promise<Stadium[]> {
    ...
    allStadiums.forEach(stadium => {
      console.log(`- All Stadium: "${stadium.name}" (status: ${stadium.status}, id: ${stadium.id})`);
    });
    ...
    results.forEach(stadium => {
      console.log(`- Matched Stadium: "${stadium.name}" (status: ${stadium.status})`);
    });
  ...
  async findByAddress(input: FindStadiumsByAddressInput): Promise<Stadium[]> {
    ...
    const stadiumsWithAddress = allStadiums.filter(stadium => 
      (stadium.address && stadium.address.trim()) || 
      (stadium.googleMap && stadium.googleMap.trim() && !stadium.googleMap.includes('maps.google.com'))
    );
    ...
    try {
      ...
      const stadiumsToProcess = stadiumsWithAddress.slice(0, maxStadiumsToProcess);
      
      console.log(`Processing ${stadiumsToProcess.length} stadiums (limited from ${stadiumsWithAddress.length})`);

      // Check distance for each stadium
      for (const stadium of stadiumsToProcess) {
        // Use address if available, otherwise use googleMap
        const stadiumAddress = stadium.address && stadium.address.trim() 
          ? stadium.address 
          : stadium.googleMap;
          
        try {
          console.log(`Calculating distance for stadium ${stadium.id}: ${stadiumAddress}`);
          
          // Add delay to avoid rate limiting
          await new Promise(resolve => setTimeout(resolve, 500)); // 500ms delay
          
          // First, geocode the stadium address to get coordinates
          const stadiumGeocodeData = await this.geocodeWithRetry(stadiumAddress!, GOONG_API_KEY);

          if (!stadiumGeocodeData.results || stadiumGeocodeData.results.length === 0) {
          ...
          try {
            ...
            const distanceResponse = await axios.get('https://rsapi.goong.io/DistanceMatrix', {
              params: {
            ...
            console.log(`Distance response for stadium ${stadium.id}:`, distanceResponse.data);
            ...
            if (distanceResponse.data.rows && 
              ...
              if (element.status === 'OK' && element.distance) {
                ...
                console.log(`Road distance for stadium ${stadium.id}: ${distanceInKm}km`);
            ...
            if (distanceError.response?.status === 429) {
              console.warn(`Rate limit exceeded for DistanceMatrix, using haversine distance for stadium ${stadium.id}`);
            } else {
              console.warn(`DistanceMatrix failed for stadium ${stadium.id}, using haversine distance`);
            }
          }
          ...
          console.log(`Haversine distance for stadium ${stadium.id}: ${haversineDistance}km`);
          ...
          if (error.response?.status === 429) {
            console.warn(`Rate limit exceeded for stadium ${stadium.id}, skipping this stadium`);
          } else {
            console.error(`Error calculating distance for stadium ${stadium.id}:`, {
      ...
      console.log(`Found ${nearbyStadiums.length} stadiums within ${radiusKm}km`);
      
      // Filter out stadiums with null address before returning
      const validStadiums = nearbyStadiums.filter(stadium => 
  ...
  private async geocodeWithRetry(address: string, apiKey: string, maxRetries = 2): Promise<any> {
    // Check cache first
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (error.response?.status === 429) {
          console.warn(`Rate limit hit on attempt ${attempt}, waiting longer...`);
